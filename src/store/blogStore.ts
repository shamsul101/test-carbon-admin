  ...
  createPost: async (post: CreateBlogPost, accessToken?: string | null) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Handle 413 error specifically (Entity Too Large)
        if (response.status === 413) {
          errorMessage =
            "Image size too large. Please select an image smaller than 1MB.";
          console.error("Create post error: 413 - Entity Too Large");
