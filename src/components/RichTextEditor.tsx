...
export default function RichTextEditor({ value, onChange, placeholder, className }: RichTextEditorProps) {
  ...
  const handleIndent = () => {
    ...
    if (node && (node as Element).tagName === 'LI') {
      ...
      // Not in a list, use regular indent
  ...
  const handleOutdent = () => {
    ...
    if (node && (node as Element).tagName === 'LI') {
      const listItem = node as HTMLLIElement;
      const parentList = listItem.parentElement;
      const grandparentItem = parentList?.parentElement?.closest('li') as HTMLLIElement | null;
      
      if (grandparentItem) {
        const grandparentList = grandparentItem.parentElement;
        
        // Find next sibling of grandparent
        ...
        // Not nested, remove from list entirely
      ...
      // Not in a list, use regular outdent
      document.execCommand('outdent', false);
      setTimeout(() => {
        if (editorRef.current) {
          onChange(editorRef.current.innerHTML);
