...
// icons
const planIcons = [
  <Star key="basic" className="w-8 h-8 text-secondary" />,
...
export default function Pricing() {
  ...
  const handleStripeCheckout = async (planId: number) => {
    if (!accessToken) {
      toast.error("You must log in to subscribe.");
      return;
    }
    if (role === "super_admin") {
    ...
    if (subscription?.payment_type === "postpaid") {
      toast.info(
        "Your plan is postpaid. Please contact the admin to complete payment."
      );
      return;
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="mx-auto">
        {/* Hero Section */}
{role === "business" && (<div className="mb-6">
...
{
  role === "individual" && (        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-4 text-slate-900">
            Ready to Go Beyond Personal Impact?
          </h1>

          <p className="text-base text-muted-foreground max-w-4xl leading-relaxed">
            Integrate real-time carbon measurement and offsetting directly into
            your platform.
          </p>
          <p className="text-base text-slate-600 max-w-4xl mt-2">
            With Emission Lab’s API, your users see emissions — and act on them
            — seamlessly.
          </p>
        </div>)
}
        ...
        <div className="relative mb-14">
          ...
          <h2 className="text-lg font-semibold text-slate-700 uppercase tracking-wide">
            {role === "individual"  && "Explore"} Our Business Plans
          </h2>
                  ...
                  <div className="flex-1">
                    ...
                    <ul className="space-y-2 md:space-y-3">
                      {plan.features.map((feature, i) => (
                        ...
                        >
                          <CheckCircle className="w-4 h-4 md:w-5 md:h-5 text-primary mt-0.5 flex-shrink-0 group-hover/item:scale-110 transition-transform duration-200" />
