...
export default function AdminDashboard() {
  ...
  const { offsetHistory, fetchOffsetHistory, fetchProjects, projects } =
    useOffsetStore();
  // console.log("offsetHistory :: ", offsetHistory);

  const projectStandards = useMemo(() => {
    const colors = [
      "#166534",
      "#15803d",
      "#16a34a",
    ...
    const standardCounts = activeProjects.reduce((acc, project) => {
      const standard = project.standard || "No Standard";
    ...
    return Object.entries(standardCounts).map(([name, count], index) => ({
      name,
  ...
  const certificatesByMonth = useMemo(() => {
    ...
    offsetHistory.forEach((item) => {
      try {
        // Parse the date_of_issue (format: "2025-11-13")
        const date = new Date(item.date_of_issue);
        const monthIndex = date.getMonth();
        ...
        console.error("Error parsing date:", item.date_of_issue, error);
      }
    });
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-700">
              {totalTonnes.toFixed(2)} metric tons
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Offset Projects Standards
            </CardTitle>
            <CardDescription>
              Distribution of carbon offset project certification standards
            </CardDescription>
          </CardHeader>
          <CardContent>
            {projectStandards.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  ...
                  >
                    {projectStandards.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.color} />
                    ))}
                  </Pie>
                  <Tooltip
                    formatter={(value, name) => [
                      `${value}% (${projectStandards.find((p) => p.name === name)?.count ||
                      0
                      } projects)`,
                      name,
                    ]}
              ...
              <div className="flex items-center justify-center h-[300px] text-muted-foreground">
                No project standards data available
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Monthly certificate issuance trends • {offsetHistory.length} total
              certificates
            </CardDescription>
          </CardHeader>
          <CardContent>
